...
class Window:
  ...
  def updateView(self):
    ...
    #print repr(ext)
    maxx = max(map(lambda x: x[0], ext['cell']))
    maxy = max(map(lambda x: x[1], ext['cell']))
    ...
    for (x, y) in ext['water']:
      self.items.append( self.canvas.create_rectangle( * fieldRect(x,y), fill=WATER_FILL) )
    for (x, y) in ext['land']:
      self.items.append( self.canvas.create_rectangle( *fieldRect(x,y,14), fill=LAND_FILL) )
    for (x, y, text) in ext['numbered']:
      fr = fieldRect(x,y)
